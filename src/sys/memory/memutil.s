/*
   Copyright 2018 Alexandru-Paul Copil

   Licensed under the Apache License, Version 2.0 (the "License");
   you may not use this file except in compliance with the License.
   You may obtain a copy of the License at

       http://www.apache.org/licenses/LICENSE-2.0

   Unless required by applicable law or agreed to in writing, software
   distributed under the License is distributed on an "AS IS" BASIS,
   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
   See the License for the specific language governing permissions and
   limitations under the License.
*/


.section .text


@ void kmemzero (void* src, u32 len)
@ Takes a source addr, replaces the next 'len' bytes with 0.
.globl kmemzero
kmemzero:
	MOV	R2, #0			@ Set value to 0
	B	kmemsetb			@ Set memory


@ kmemcopy (void* src, u32 len, void* dst)
@ Takes a source addr, copies the first 'len' bytes from src to dst.
.globl kmemcopy
kmemcopy:
	CMP	R1, #0			@ Check invalid arguments
	BXEQ	LR			@

	CMP	R2, R0			@ If src > dst
	ADDHI	R0, R1			@  dst += len
	ADDHI	R2, R1			@  src += len
	BHI	_kmemcopy_1		@

 _kmemcopy_0:				@ Bottom to top
	LDRB	R3, [R0], #1		@ dst++ = src++
	STRB	R3, [R2], #1		@
	SUBS	R1, #1			@ while (len-- > 0)
	BNE	_kmemcopy_0		@   repeat
	BX	LR			@ Return

 _kmemcopy_1:				@ Top to bottom
	LDRB	R3, [R0, #-1]!		@ --dst = --src
	STRB	R3, [R2, #-1]!		@
	SUBS	R1, #1			@ while (len-- > 0)
	BNE	_kmemcopy_1		@   repeat
	BX	LR			@ Return


@ void kmemsetb (void* addr, u32 len, u8 value)
@ Set the next `len` bytes to to value.
.globl kmemsetb
kmemsetb:
	TEQ	R1, #0			@ Check for invalid arguments
	BXEQ	LR			@

	TST	R0, #2			@ Check if 4 byte aligned
	LSREQ	R1, #2			@ Transform byte len to word len
	ORREQ	R2, R2, R2, LSL #8	@ Fill word with 4xbyte
	ORREQ	R2, R2, R2, LSL #16	@
	BEQ	_kmemsetw		@ Fill space as words

	TST	R0, #1			@ Check if 2 byte aligned
	LSREQ	R1, #1			@ Transform byte len to hword len
	ORREQ	R2, R2, R2, LSL #8	@ Fill hword with 2xbyte
	BEQ	_kmemseth		@ Fill space as words

 _kmemsetb:
	STRB	R2, [R0], #1		@ Write byte
	SUBS	R1, #1			@ Decrement
	BNE	_kmemsetb		@ Repeat
	BX	LR			@ Return


@ void kmemseth (void* addr, u32 len, u16 value)
@ Set the next `len` hwords to to value.
.globl kmemseth
kmemseth:
	TEQ	R1, #0			@ Check for invalid arguments
	BXEQ	LR			@

	TST	R0, #2			@ Check if 4 byte aligned
	LSREQ	R1, #1			@ Transform hword len to word len
	ORREQ	R2, R2, R2, LSL #16	@ Fill word with 2xhword
	BEQ	_kmemsetw		@ Fill space as words

 _kmemseth:
	STRH	R2, [R0], #2		@ Write hword
	SUBS	R1, #1			@ Decrement
	BNE	_kmemseth		@ Repeat
	BX	LR			@ Return

	BX	LR			@ Return


@ void kmemsetw (void* addr, u32 len, u32 value)
@ Set the next `len` words to to value.
.globl kmemsetw
kmemsetw:
	TEQ	R1, #0			@ Check for invalid arguments
	BXEQ	LR			@

 _kmemsetw:
	STR	R2, [R0], #4		@ Write hword
	SUBS	R1, #1			@ Decrement
	BNE	_kmemsetw		@ Repeat
	BX	LR			@ Return

	BX	LR			@ Return
